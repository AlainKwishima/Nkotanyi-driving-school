...
export function ExamNativeScreen({ navigation, route }: Props) {
  ...
  const performExamSubmit = useCallback(async () => {
    ...
    // Move immediately so the tap feels responsive on-device, then persist the record in the background.
    if (passed) {
    ...
    if (!hasSubscription && !hasUsedFreeTrial) {
      void setHasUsedFreeTrial(true);
    }

    void appendLocalExamRecord({
      correct,
      total,
      percent,
      timeLabel,
      ...
      console.warn('Failed to store exam history', error);
    });
  }, [
    elapsedSec,
    hasSubscription,
          ...
          {current.options.map((opt, index) => {
            ...
            return (
              <TouchableOpacity
                ...
                activeOpacity={0.85}
              >
                <View style={[styles.optionLetterBox, active && styles.optionLetterBoxSelected]}>
                  <Text style={[styles.optionLetterText, active && styles.optionLetterTextSelected]}>{letter}</Text>
                </View>
